...
const updateRefreshTokenRevoke = `-- name: UpdateRefreshTokenRevoke :one
...
func (q *Queries) UpdateRefreshTokenRevoke(ctx context.Context, token string) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, updateRefreshTokenRevoke, token)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
